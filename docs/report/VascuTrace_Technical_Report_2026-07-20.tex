...
\hypersetup{
  ...
  pdfauthor={Venkat Sumanth Reddy Bommireddy},
...
The machine-readable evidence ledger accompanies this report. It contains aggregate values, source classes, hashes of evidence inputs, limitations, and figure provenance. It contains no case row, identifier, medical image, credential, model weight, or local absolute path.
...
The report uses only aggregate tables, public primary sources, current public product code, and generated schematics. Raw volumes, headers, identifiers, per-case rows, case screenshots, model weights, caches, and run outputs were excluded. Every public figure was generated from the aggregate evidence ledger or from a fully synthetic schematic. This publication turn did not reopen the medical images.
...
\begin{longtable}{>{\bfseries}p{0.14\textwidth} p{0.24\textwidth} p{0.24\textwidth} p{0.21\textwidth}}
...
The figure builder reads only the aggregate JSON ledger and writes eight PNG files. It does not read the dataset, caches, or weights. The report build uses \texttt{pdflatex}, BibTeX, and two final LaTeX passes. The expected commands are listed in Appendix~\ref{app:commands}.

\subsection{Data separation}

Raw medical volumes, subject identifiers, session manifests, per-case results, model weights, and run outputs remain local and outside the publication snapshot. The PDF uses no medical slice. Figure captions identify each population and independent unit without exposing a subject or file name. The retrieval index is also excluded because its earlier corpus was not publication-safe.
...
\begin{longtable}{>{\ttfamily}p{0.12\textwidth} p{0.19\textwidth} p{0.24\textwidth} p{0.24\textwidth}}
...
The following commands define the publication build and the project-level checks expected before release. A command should be reported as passed only after a fresh run with an exit code of zero.
